...
let getFreeSeat (occupiedSeats : int list) =
    let possibleNumbers = [0 .. 835]
    possibleNumbers 
        |> List.filter (fun seatNumber ->
            if not (occupiedSeats |> List.exists (fun x -> x = seatNumber)) &&
                occupiedSeats |> List.exists (fun x -> x = seatNumber - 1 ) &&
                occupiedSeats |> List.exists (fun x -> x = seatNumber + 1 ) 
                then
                true
            else
